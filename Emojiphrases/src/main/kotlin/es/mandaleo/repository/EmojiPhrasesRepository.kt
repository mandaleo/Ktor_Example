package es.mandaleo.repository

import es.mandaleo.model.*
import es.mandaleo.repository.DatabaseFactory.dbQuery
...
class EmojiPhrasesRepository: Repository {
    ...
    override suspend fun phrases(): List<EmojiPhrase> {
        return dbQuery {
            EmojiPhrases.selectAll().map { toEmojiPhrases(it) }
        }
    }
    ...
    override suspend fun clear() {
        dbQuery { EmojiPhrases.deleteAll() }
    }
    ...
    override suspend fun createUser(user: User) {
        dbQuery {
            Users.insert {
                it[id] = user.userId.toString()
                it[displayName] = user.displayName.toString()
                it[email] = user.email.toString()
                it[passwordHash] = user.passwordHash.toString()
            }
        }
    }

    private fun toEmojiPhrases(row: ResultRow): EmojiPhrase {
        return EmojiPhrase(
