...
fun Route.phrases(db: Repository, hasFunction: (String) -> String) {
    get<Phrases> {
        ...
        if (user == null) {
            ...
            val code = call.securityCode(date, user, hasFunction)
    ...
    post<Phrases> {
        ...
        if (user == null || !call.verifyCode(date, user, code, hasFunction)) {
            call.redirect(Signin())
        }
